...
from DataBaseOperate.setting import anno_engine, spid_engine, rel_engine
...
class BaseTable:
    ...
    def getSession(cls):
        ...
        :return:
        """
        if not cls.__DBSession:
            if hasattr(cls, 'metadata'):
                cls.__DBSession = sessionmaker(bind=cls.metadata.bind)
            else:
                raise AttributeError('未继承Base类')
    ...
    def create_table(cls):
        ...
        warnings.warn(
            """
            please delete it when you used \n this may be create table in any database
            before it updated from multiple extends to single extend, don't use it in running
            """,
            DeprecationWarning)
        if hasattr(cls, 'metadata'):
...
class AnTable(BaseTable, declarative_base(bind=anno_engine)):
    """
    连接爬取公告数据库
...
class SMTable(BaseTable, declarative_base(bind=spid_engine)):
    """
    连接爬虫管理系统数据库
    """
    __abstract__ = True
...
class RelTable(BaseTable, declarative_base(bind=rel_engine)):
    """
    连接分析数据库
    """
    __abstract__ = True

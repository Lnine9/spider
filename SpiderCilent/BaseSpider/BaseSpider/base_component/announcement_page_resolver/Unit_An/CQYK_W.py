...
class CQYK_W(HtmlPageResolver):
    ...
    def getWBContent(self):
        ...
        content['ancm_time'] = get_one_time_from_str(
            resp.xpath(".//div[@class='addtime']/span[1]/text()").extract_first())
        ...
        content['provide_unit'] = util.find_in_list(html_list, '中标供应商：(.+(?:公司))')
        content['provide_address'] = ''
        content['actual_price'] = util.find_in_list(html_list, '投标报价：(.+(?:元))')

...
class XNYY_W(HtmlPageResolver):
    ...
    def resolver_page(self) -> dict:
        ...
        self.resp = resp
        try:
            title = resp.xpath("//h2/text()").extract_first()
            if not title:
                return {}
    ...
    def getWBContent(self):
        resp = self.resp.xpath("//form[@name='_newscontent_fromname']")
        ...
        content['title'] = self.resp.xpath("//h2/text()").extract_first()
        content['proj_name'] = re.sub(r'采购结果|中标公告|成交公告|成交|结果|公告', '', content['title'])
        content['proj_item'] = ''
        content['ancm_time'] = get_one_time_from_str(
            resp.xpath(".//div[@class='Author']/span[1]/text()").extract_first())
        content['sourse_url'] = self.response_url
        content['web_site'] = 'http://www.xnyy.cn/'
        content['purchase_m'] = content['purchase_m'] if content['purchase_m'] else '公开招标'
        content['source_web_name'] = '西南医院'
        content['region'] = '重庆市.沙坪坝区'
        content['call_unit'] = '重庆市西南医院'
        content['call_unit_address'] = '重庆市沙坪坝区西南医院'
        content['agent_unit_name'] = '重庆市西南医院'
        content['agent_unit_address'] = '重庆市沙坪坝区西南医院'

        content['proj_code'] = util.search_code(html_list)
        content['provide_unit'] = resp.xpath(
            "//div[@class='v_news_content']//table//tr[2]/td[2]//span/text()").extract_first()
        content['provide_address'] = ''
        heads = resp.xpath("//div[@class='v_news_content']//table//tr[1]/td//span/text()").extract()
        for i, h in enumerate(heads):
            if re.search('元', h):
                content['actual_price'] = resp.xpath("//div[@class='v_news_content']//table//tr[2]/td[{}]//span/text()".format(i+1)).extract_first()

                money_head = resp.xpath("//div[@class='v_news_content']//table//tr[1]/td[{}]//span/text()".format(i+1)).extract_first()
                if content['actual_price']:
                    if money_head:
                        if re.search(r'万', money_head):
                            content['actual_price'] = content['actual_price'] + '万元'
                        else:
                            content['actual_price'] = content['actual_price'] + '元'
                    else:
                        content['actual_price'] = content['actual_price'] + '万元'
                break

...
class XNZF(HtmlPageResolver):
    ...
    def resolver_page(self) -> dict:
        ...
        self.resp = resp
        try:
            title = resp.xpath("//div[@class='articleTitle']/h2/text()").extract_first()
            print(title)
            if not title:
                return {}
    ...
    def getCodeHtml(self):
        ...
        code_dict['code'] = self.resp.xpath("//div[@class='pageArticle']").get()

        return code_dict

    def getCBContent(self):
        resp = self.resp.xpath("//div[@class='pageArticle']")
        ...
        # 复杂数据获取
        content['title'] = self.resp.xpath(".//div[@class='articleTitle']/h2/text()").extract_first()
        content['proj_name'] = re.sub(r'招标公告|采购公告|公告', '', content['title'])
        content['proj_item'] = ''
        content['ancm_time'] = get_one_time_from_str(resp.xpath(".//div[@class='articleAuthor']/text()").extract_first())
        content['sourse_url'] = self.response_url
        content['web_site'] = 'https://jhc.swupl.edu.cn/tzgg/'
        content['purchase_m'] = content['purchase_m'] if content['purchase_m'] else '公开招标'
        content['source_web_name'] = '西南政法大学'
        content['region'] = '重庆市.沙坪坝区'
        content['call_unit'] = '重庆市西南政法大学'
        content['call_unit_address'] = '重庆市沙坪坝区西南政法大学'
        content['agent_unit_name'] = '重庆市西南政法大学'
        content['agent_unit_address'] = '重庆市沙坪坝区西南政法大学'

...
def find_in_list(list, regx):
    for row in list:
        res = re.search(regx, row)
        if res:
            return res.group(1)
    return None
...
def parse2dict_title_with_dig(list, regx=r'^[一二三四五六七八九十\d](?:\d|\s|一|二|三|)[、\.．]', except_regx=r'^\d+\.\d+'):
    dict = {}
    last_index = -1
    last_title = ''
...
def search_code(list):
    for row in list:
        if re.search('电话|联系方式', row):
            continue
        codes = re.search(r'[0-9a-zA-Z()（）\-/\]\[]{9,}', row)
        if codes:
            code = codes.group()
...
def process_callbid_file(dict: dict):
    ...
    for key in dict.keys():
        if re.search('标书|招标文件|获取.*文件|报价.*文件|报名.*文件', key):
...
def process_callbid_tender(dict: dict):
    if not dict:
        return {}
    for key in dict.keys():
        if re.search('投标(?!人|须知|说明)|报名', key):
...
def process_contact_simple(dict):
    if not dict:
        return {}
    item = {}
    for key in dict.keys():
        if re.search('联系|采购人|招标人', key):
            rows = dict[key]
            dict2 = parse2dict(rows)
            item['agent_unit_name'] = ''
...
def process_contact_from_all(dict):
    if not dict:
        return {}
    item = {}
    for key in dict.keys():
        ...
        item['agent_unit_p'] = ''
        item['agent_unit_m'] = ''
        item['call_unit'] = match(dict2, '招标人|采购人')
        item['call_unit_address'] = match(dict2, '地址')
        item['proj_rel_p'] = match(dict2, '联系人')
        item['proj_rel_m'] = match(dict2, '电话|邮箱')
        return item
    return {}

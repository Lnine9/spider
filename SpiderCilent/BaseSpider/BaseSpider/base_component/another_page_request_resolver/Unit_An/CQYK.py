...
class CQYK(RequestResolver):
    ...
    def general_param(self) -> dict:
        ...
        if not total_page:
            resp = Selector(text=requests.get('https://www.hospital-cqmu.com/gzb/zbgg1.htm').text)
            total_page = int(resp.xpath("(//a[@href='1.htm'])[1]/text()").extract_first())
            self.total_page = total_page

        if self.req_attr.page_num == 1:
            url = 'https://www.hospital-cqmu.com/gzb/zbgg/'
        else:
            url = 'https://www.hospital-cqmu.com/gzb/zbgg/' + str(total_page - self.req_attr.page_num + 1) + '.htm',

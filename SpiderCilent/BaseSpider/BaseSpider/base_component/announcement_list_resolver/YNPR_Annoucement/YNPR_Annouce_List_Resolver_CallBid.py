...
CONST_PARAM = {
    ...
    # 当前最大网页
    'LARGEST_PAGE': '//*[@id="zbggForm"]/div[3]/div/div/a[last()-1]',
    # 当前网页最新URl
    'CUR_LATEST_URL': '//*[@id="data_tab"]/tbody/tr[2]/td[3]/a/@href',
    # URL列表XPATH路径
    'LINK_HREFS': '//*[@id="data_tab"]/tbody/tr/td[3]/a/@href',
    # 当前页面页码
    'AIM_CRAWL_PAGE': '//*[@id="zbggForm"]/div[3]/div/div/a[@class="one"]/text()',
}


class YNPR_Annouce_List_Resolver_CallBid(PageResolver):
    #  该init方法必须在子类（该类）实现
    def __init__(self):
    ...
    def resolver_page(self) -> PageAttribute:
        ...
        for item in self.response.xpath('//*[@id="data_tab"]/tbody/tr'):
            if item.xpath('./td[3]/a/@href').get() is not None:
                page_size += 1
                act_url = CONST_PARAM.get('BASE_URL') + item.xpath('./td[3]/a/@href').get()

...
class YN_MoBid_sub_component1(SubResolver):
    ...
    def resolver_page(self) -> dict:
        try:
            ...
            if self.response_text.xpath('//*[@id="jggglx"]/label[3]/input/@checked').get() == 'checked':
                return None
            content = self.getModifyBidGovernment()
    ...
    def getModifyBidGovernment(self):
        ...
        for each in self.response_text.xpath('//*[@id="searchPanel"]/div'):
            ...
            content[key] = value
        content['首次公告日期'] = self.response_text.xpath('//*[@id="cfp"]/div[1]/div/text()').get()
        content['行政区域'] = '云南省'
        if content['机构项目编号'] == content['项目名称']:
            content['项目名称'] = content['公告标题']
        # 匹配关键字段
        content = process_dict(self.annoucement_type,content)
        if content['origin_announce_time'] == '':
            content['origin_announce_time'] = None

...
from BaseSpider.base_component.announcement_page_resolver.ZGZF_Annoucement.table_contents import \
    transverse_table_contents, simple_direction_table_contents, obtain_td_matrix
...
class ZGZF_Announce_Resolver_Deal_component1(SubResolver):
    def __init__(self):
        self.all_item = {}
    ...
    def getWinBidGovernment(self):
        ...
        if other_info == '':
            other_info = search_all_by_xpath('其它事宜', '采购项目联系人', response, 'p', '', 2)
    ...
    def getProvideUnit(self):
        ...
        if len(removal_provide_unit) == len(removal_provide_address):
            for i in range(len(removal_provide_unit)):
                regex = re.compile(r'(^null$|^无$|^/$|^-$|^——$|^--$|.*详见.*|.*见附件.*|)')
    ...
    def getUndefinedExp(self):
        ...
        if experts == '':
            experts = getValue.get_one_from_dict(getValue.search_similar_val('谈判小组、询价小组成员、磋商小组成员名单及单一来源采购人员名单', self.all_item))
            if experts == '':
    ...
    def getAgentUnit(self):
        ...
        item = {'code': self.content['agent_unit_name'],
                'name': self.content['agent_unit_name'],
                'address': self.content['agent_unit_address']}

...
class ZGZF_Announce_Resolver_Inquiry_component1(SubResolver):
    def __init__(self):
    ...
    def getInquiryGovernment(self):
        ...
        if content['bid_sale_m'] == '':
            content['bid_sale_m'] = search_all_by_xpath('获取询价文件方式', '获取询价文件文件售价', response, 'p', '')

        if content['bid_price'] == '':
            content['bid_price'] = search_all_by_xpath('获取询价文件文件售价', '响应文件时间及地点等', response, 'p', '')
        ...
        content['bid_sale_op_time'] = get_one_time_from_str(content['ancm_time'])
        content['bid_sale_en_time'] = get_one_time_from_str(content['ancm_time'])

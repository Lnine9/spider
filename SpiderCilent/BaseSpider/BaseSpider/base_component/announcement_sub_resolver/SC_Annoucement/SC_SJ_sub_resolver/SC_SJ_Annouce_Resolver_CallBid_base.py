...
class SC_SJ_Annouce_Resolver_CallBid_base(SubResolver):
    """公告页请求解析器"""
    ...
    def getCallBidGovernment(self):
        ...
        if p == "":
            p = messageDict.get("采购项目联系人姓名和电话", None)
        if m == "":
            m = messageDict.get("采购项目联系人姓名和电话", None)
        ...
        if type == "竞争性磋商采购":
            ...
            self.content["bid_sale_place"] = messageDict.get("磋商文件发售及供应商报名地点", None)
            self.content["bid_sale_m"] = messageDict.get("磋商文件发售方式", None)
            self.content["bid_sale_op_time"] = trans_time(messageDict.get("获取磋商文件开始时间", None))
            self.content["bid_sale_en_time"] = trans_time(messageDict.get("获取磋商文件结束时间", None))
            self.content["bid_end_time"] = trans_time(messageDict.get("响应文件递交结束时间", ""))
            self.content["tender_place"] = messageDict.get("响应文件递交地点", None)
            self.content["bid_time"] = trans_time(messageDict.get("响应文件开启时间", ""))
        ...
        if type == "公开招标":
            ...
            self.content["bid_end_time"] = trans_time(messageDict.get("投标截止时间", None))
            self.content["tender_place"] = messageDict.get("投标地点", None)
            self.content["bid_time"] = trans_time(messageDict.get("开标时间", None))
        ...
        if type == "竞争性谈判":
            ...
            if messageDict.get("谈判文件起售时间", None) is None:
                self.content["bid_sale_op_time"] = trans_time(messageDict.get("谈判文件发售及供应商报名时间").split("到")[0])
                self.content["bid_sale_en_time"] = trans_time(messageDict.get("谈判文件发售及供应商报名时间").split("到")[1])
            else:
                self.content["bid_sale_op_time"] = trans_time(messageDict.get("谈判文件起售时间", None))
                self.content["bid_sale_en_time"] = trans_time(messageDict.get("谈判文件止售时间", None))
            self.content["tender_place"] = messageDict.get("谈判响应文件递交地点", None)
            if messageDict.get("谈判响应文件开启时间", None) is None:
                self.content["bid_time"] = trans_time(messageDict.get("供应商接受资格审查及参加谈判时间"))
                self.content["bid_place"] = messageDict.get("供应商接收资格审查及参加谈判地点")
            else:
                self.content["bid_time"] = trans_time(messageDict.get("谈判响应文件开启时间", None))
                self.content["bid_place"] = messageDict.get("谈判响应文件开启地点", None)
            if messageDict.get("谈判响应文件递交截止时间") is None:
                self.content["bid_end_time"] = trans_time(messageDict.get("供应商递交响应文件起止时间", None).split("到")[1])
            else:
                self.content["bid_end_time"] = trans_time(messageDict.get("谈判响应文件递交截止时间", None))
        ...
        if type == "单一来源采购":
            ...
            self.content["bid_end_time"] = trans_time(messageDict.get("洽谈时间", None))
            self.content["tender_place"] = messageDict.get("洽谈地点", None)
            self.content["bid_time"] = trans_time(messageDict.get("洽谈时间", None))
        ...
        if type == "询价采购":
            ...
            self.content["bid_end_time"] = trans_time(messageDict.get("供应商递交响应文件起止时间", "").split("到")[1])
            self.content["tender_place"] = trans_time(messageDict.get("供应商递交响应文件地点", None))
